...
int CRingBuffer::Enqueue(char* chpData, int iSize)
{
	...
	if (sizeCheck >= queueLen)
	{
		...
		this->rear = (this->rear + directSize) % queueLen;

		int addsize = iSize - directSize;

		memcpy(&this->mRingBuffer[(this->rear + 1) % queueLen], &chpData[directSize], addsize);

		this->rear = (this->rear + addsize) % queueLen;
...
{
	int pDirectSize = GetUseSize();

	if (iSize > pDirectSize)
	{
		iSize = pDirectSize;
	}
	...
	int dummyF = this->front;

	for (int iCnt = 0; iCnt < iSize; ++iCnt)
	{
		dummyF = (dummyF + 1) % queueLen;

		chpDest[iCnt] = this->mRingBuffer[dummyF];
	}
	return iSize;
}
...
void CRingBuffer::MoveRear(int iSize)
{
	for (int iCnt = 0; iCnt < iSize; ++iCnt)
	{
		...
		this->rear = (this->rear - 1) % this->mQueueLen;
	}
}
...
void CRingBuffer::MoveFront(int iSize)
{
	for (int iCnt = 0; iCnt < iSize; iCnt++)
	{
		...
		this->front = (this->front + 1) % this->mQueueLen;
	}
}
