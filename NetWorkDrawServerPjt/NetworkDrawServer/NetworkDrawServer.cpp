// NetworkDrawServer.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
...
{
    SOCKET clientSock;
    CRingBuffer sendRqueue;
    CRingBuffer recvRqueue;
};
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_NETWORKDRAWSERVER, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_NETWORKDRAWSERVER));
    ...
    retval = setsockopt(listenSock, SOL_SOCKET, SO_LINGER, (char*)&opt, sizeof(opt));
    ...
    retval = WSAAsyncSelect(listenSock, hWnd, WM_NETWORK, FD_ACCEPT | FD_CLOSE);
    if (retval == SOCKET_ERROR)
    {
        printf_s("async select error : %d\n", WSAGetLastError());
        return -1;
    }
    ...
    if (retval == SOCKET_ERROR)
    {
        printf_s("listen error : %d", WSAGetLastError());
        return-1;
    }
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_NETWORKDRAWSERVER));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NETWORK:
        ...
        {
            printf_s("WM_NETWORK error : %d", WSAGetLastError());
        ...
        case FD_ACCEPT:
        {
            ...
            InetNtopA(AF_INET, &clientAddr.sin_addr, test, sizeof(test));

            sockSu++;

            printf_s("접속 IP : %s, 접속 소켓 수 : %d\n", test,sockSu);

            WSAAsyncSelect(clientSock, hWnd, WM_CLIENT, FD_READ | FD_WRITE | FD_CLOSE);
...
void ReadEvent()
{
    ...
    for (CList<Session*>::Iterator iter = sessionList.begin(); iter != iterE; ++iter)
    {
        ...
        int enSize;
        enSize = iter->recvRqueue.Enqueue(buffer, retval);
        if (enSize != retval)
        {
            Disconnect(iter->clientSock);
            continue;
        ...
        while (1)
        {
            if (iter->recvRqueue.GetUseSize() < 2) 
            {
                break;
            }


            retval = iter->recvRqueue.Peek((char*)&header, 2);
            if (retval != 2)
            {
                printf_s("peek error\n");
                Disconnect(iter->clientSock);
            ...
            if (header.lEn + 2 > iter->recvRqueue.GetUseSize())
            {
                break;
            }

            iter->recvRqueue.MoveFront(2);

            retval = iter->recvRqueue.Dequeue((char*)&drawPacket, header.lEn);
            ...
            WriteBuffer((*iter)->data,header, drawPacket, header.lEn);
